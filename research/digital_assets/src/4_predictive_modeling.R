...
# XGBoost is short for "Extreme Gradient Boosting", a popular predictive 
...
# normally decision trees. For XGBoost, this technique uses a more regularised
# model formalisation to control over-fitting, a common problem with decision
# tree models.
...
if (!refresh_flag){
  xgb_model_trained <- readRDS(file.path(path_output, "xgb_model_trained.rds"))
} else {
  ...
  saveRDS(xgb_model_trained, file=file.path(path_output, "xgb_model_trained.rds"))
}
...
xgboost_score_test <- test_prediction_xgb %>% 
...
model_summaries$Model <- c(
  rep("XGBoost", 2), rep("Catboost", 2), rep("Log Reg", 2)
)
...
stratcomp_xgb <- test_prediction_xgb %>% 
...
stratcomp_xgb_cost <- test_prediction_xgb %>% 
  left_join(bitcoin_return) %>%
  mutate(.pred_class = as.numeric(.pred_class),
         `Buy & Hold` = ROC(close, n = 1) %>% lead(), 
         `XGB Strategy` = .pred_class*`Buy & Hold`,
...
save(
  ...
  file=file.path(path_output, "predictive_modeling.RData"))
#----------------------------------------------------------------------------
#----------------------------------------------------------------------------
rm(data_split)
...
rm(xgb_model, xgb_model_best, xgb_model_trained)
rm(xgboost_score_test, xgboost_score_train)
rm(xgb_wflw, xgb_wflw_best)
rm(xgb_params)
...
rm(stratcomp_xgb, stratcomp_xgb_cost)
rm(stratcomp_cat, stratcomp_cat_cost)
rm(stratcomp_log, stratcomp_log_cost)

rm(train, train_processed)
rm(train_prediction_cat, train_prediction_log, train_prediction_xgb)

rm(test, test_processed) 
rm(test_prediction_cat, test_prediction_log, test_prediction_xgb)
