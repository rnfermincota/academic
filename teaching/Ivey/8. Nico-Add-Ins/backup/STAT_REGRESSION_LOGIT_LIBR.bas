Attribute VB_Name = "STAT_REGRESSION_LOGIT_LIBR"
...
'************************************************************************************
'************************************************************************************
'FUNCTION      : LOGIT_NLLS_FUNC
'DESCRIPTION   : Non Linear Least Square (logit)
'LIBRARY       : STATISTICS
'GROUP         : REGRESSION_LOGIT
'ID            : 001
...
Function LOGIT_NLLS_FUNC(ByRef XDATA_RNG As Variant, _
...
'Check for convergence (small values for changes in the coefficients
'or small changes in SSR).
...
'-------------------------------------------------------------------------------------------
    ...
    For i = 1 To NROWS
        ...
        Next j
        PREDICT_VECTOR(i, 1) = Exp(YFIT_VAL) / (1 + Exp(YFIT_VAL))
        ...
        PHI_VECTOR(i, 1) = PREDICT_VECTOR(i, 1) / (1 + Exp(YFIT_VAL))
        For j = 1 To NCOLUMNS
...
'-------------------------------------------------------------------------------------------
ERROR_LABEL:
LOGIT_NLLS_FUNC = Err.number
End Function

'************************************************************************************
'************************************************************************************
'FUNCTION      : LOGIT_ML_FUNC
'DESCRIPTION   : Logit Maximum Likelihood Results For Dependent Variable
'LIBRARY       : STATISTICS
'GROUP         : REGRESSION_LOGIT
'ID            : 002
'AUTHOR        : RAFAEL NICOLAS FERMIN COTA
...
Function LOGIT_ML_FUNC(ByRef XDATA_RNG As Variant, _
ByRef YDATA_RNG As Variant, _
Optional ByVal INTERCEPT_FLAG As Boolean = True)
...
'-----------------Computing LnBeta vector and Hessian matrix-------------
    For i = 1 To NROWS
        TEMP_VAL = Exp(-YFIT_VECTOR(i, 1)) / (1 + Exp(-YFIT_VECTOR(i, 1)))
        PHI_VECTOR(i, 1) = Exp(YFIT_VECTOR(i, 1)) / (1 + Exp(YFIT_VECTOR(i, 1))) ^ 2
        For k = 1 To NCOLUMNS
            'TEMP_VECTOR(k,1) = TEMP_VECTOR(k,1) + (YTEMP(i, 1) - (Exp(YFIT_VECTOR(i, 1)) / (1 + Exp(YFIT_VECTOR(i, 1))))) * XTEMP(i, k)
            TEMP_VECTOR(k, 1) = TEMP_VECTOR(k, 1) + (Y_VECTOR(i, 1) * TEMP_VAL + (1 - Y_VECTOR(i, 1)) * (TEMP_VAL - 1)) * X_MATRIX(i, k)
            For j = 1 To NCOLUMNS
                TEMP_MATRIX(j, k) = TEMP_MATRIX(j, k) + -PHI_VECTOR(i, 1) * X_MATRIX(i, j) * X_MATRIX(i, k)
            Next j
        Next k
        LN1_VAL = LN1_VAL + Y_VECTOR(i, 1) * YFIT_VECTOR(i, 1) - Log(1 + Exp(YFIT_VECTOR(i, 1)))
    Next i
'-------------------------------------------------------------------------
'---------------------------Invert Hessian--------------------------------
    TEMP_MATRIX = MATRIX_INVERSE_FUNC(TEMP_MATRIX, 0)
    If NCOLUMNS = 1 Then
        FACTOR_VAL = TEMP_MATRIX(1, 1) * TEMP_VECTOR(1, 1)
        For k = 1 To NCOLUMNS
...
'If DELTA_VAL > tolerance Then GoTo ERROR_LABEL 'Convergence not achieved.

BASE_VAL = 0
FIT_VAL = Log(YMEAN_VAL / (1 - YMEAN_VAL))
For i = 1 To NROWS
    BASE_VAL = BASE_VAL + Y_VECTOR(i, 1) * FIT_VAL - Log(1 + Exp(FIT_VAL))
Next i
RSQ_VAL = 1 - LN1_VAL / BASE_VAL
...
LOGIT_ML_FUNC = ANOVA_MATRIX

Exit Function
ERROR_LABEL:
LOGIT_ML_FUNC = Err.number
End Function
